...
@Slf4j
@Singleton
public class XmrBSFormatter implements CoinFormatter {
    ...
    public static String getCurrencyPair(String currencyCode) {
        if (CurrencyUtil.isFiatCurrency(currencyCode))
            return "XMR" + "/" + currencyCode;
        else
            return currencyCode + "/" + "XMR";
    }
    ...
    public static String getPriceWithCurrencyCode(String currencyCode, String translationKey) {
        if (CurrencyUtil.isCryptoCurrency(currencyCode))
            return Res.get(translationKey, "XMR", currencyCode);
        else
            return Res.get(translationKey, currencyCode, "XMR");
    }

    public static String getDirectionWithCodeDetailed(OfferPayload.Direction direction, String currencyCode) {
        if (CurrencyUtil.isFiatCurrency(currencyCode))
            return (direction == OfferPayload.Direction.BUY) ? Res.get("shared.buyingWith", Trade.TradeBaseCurrency.XMR, currencyCode) : Res.get("shared.sellingFor", Trade.TradeBaseCurrency.XMR, currencyCode);
        else
            return (direction == OfferPayload.Direction.SELL) ? Res.get("shared.buyingCurrency", currencyCode, Trade.TradeBaseCurrency.XMR) : Res.get("shared.sellingCurrency", currencyCode, Trade.TradeBaseCurrency.XMR);
    }
    ...
    @NotNull
    public String formatCoin(XmrCoin coin, int decimalPlaces) {
    ...
    public String formatCoin(XmrCoin coin, int decimalPlaces, boolean decimalAligned, int maxNumberOfDigits) {
        return formatCoin(coin, decimalPlaces, decimalAligned, maxNumberOfDigits, monetaryFormat);
    }

    public static String formatCoin(XmrCoin coin,
                                    int decimalPlaces,
    ...
    public String formatCoinWithCode(XmrCoin coin) {
    ...
    public static String getDateFromBlockHeight(long blockHeight) {
        ...
        return XmrBSFormatter.formatDateTime(new Date(now + blockHeight * 10 * 60 * 1000L), dateFormatter, timeFormatter);
    }
	...
	public String formatCoin(Monetary coin) {
		return formatCoin((XmrCoin) coin);
	}

	@Override
	public String formatCoinWithCode(Monetary coin) {
		return formatCoinWithCode((XmrCoin) coin);
	}
}
