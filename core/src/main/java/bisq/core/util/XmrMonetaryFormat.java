 ...
 * 
 * <p>
 * XmrMonetaryFormat instances are immutable. Invoking a configuration method has no effect on the receiving instance; you
 * must store and use the new instance it returns, instead. Instances are thread safe, so they may be stored safely as
 * static constants.
 * </p>
 */
public class XmrMonetaryFormat {

    /** Standard format for the XMR denomination. */
    public static final XmrMonetaryFormat XMR = new XmrMonetaryFormat().shift(0).minDecimals(2).repeatOptionalDecimals(2, 3);
    /** Standard format for fiat amounts. */
    public static final XmrMonetaryFormat FIAT = new XmrMonetaryFormat().shift(0).minDecimals(2).repeatOptionalDecimals(2, 1);
    /** Currency code for base 1 Monero. */
    public static final String CODE_XMR = "XMR";
     ...
     */
    public XmrMonetaryFormat negativeSign(char negativeSign) {
        checkArgument(!Character.isDigit(negativeSign));
        checkArgument(negativeSign > 0);
        if (negativeSign == this.negativeSign)
            return this;
        else
            return new XmrMonetaryFormat(negativeSign, positiveSign, zeroDigit, decimalMark, minDecimals, decimalGroups,
     ...
     */
    public XmrMonetaryFormat positiveSign(char positiveSign) {
     ...
     */
    public XmrMonetaryFormat digits(char zeroDigit) {
     ...
     * used either.
     */
    public XmrMonetaryFormat decimalMark(char decimalMark) {
     ...
     * parsing.
     */
    public XmrMonetaryFormat minDecimals(int minDecimals) {
     ...
     */
    public XmrMonetaryFormat optionalDecimals(int... groups) {
     ...
     */
    public XmrMonetaryFormat repeatOptionalDecimals(int decimals, int repetitions) {
    ...
    /**
     * Set number of digits to shift the decimal separator to the right. Note this will change the currency code if enabled.
     */
    public XmrMonetaryFormat shift(int shift) {
        if (shift == this.shift)
            return this;
        else
            return new XmrMonetaryFormat(negativeSign, positiveSign, zeroDigit, decimalMark, minDecimals, decimalGroups,
     ...
     */
    public XmrMonetaryFormat roundingMode(RoundingMode roundingMode) {
     ...
     */
    public XmrMonetaryFormat noCode() {
        if (codes == null)
            return this;
        else
            return new XmrMonetaryFormat(negativeSign, positiveSign, zeroDigit, decimalMark, minDecimals, decimalGroups,
     ...
     *            currency code
     */
    public XmrMonetaryFormat code(int codeShift, String code) {
     ...
     */
    public XmrMonetaryFormat codeSeparator(char codeSeparator) {
     ...
     */
    public XmrMonetaryFormat prefixCode() {
        if (codePrefixed)
            return this;
        else
            return new XmrMonetaryFormat(negativeSign, positiveSign, zeroDigit, decimalMark, minDecimals, decimalGroups,
     ...
     */
    public XmrMonetaryFormat postfixCode() {
        if (!codePrefixed)
            return this;
        else
            return new XmrMonetaryFormat(negativeSign, positiveSign, zeroDigit, decimalMark, minDecimals, decimalGroups,
     ...
     */
    public XmrMonetaryFormat withLocale(Locale locale) {
    ...
    public XmrMonetaryFormat() {
        ...
        this.codes[0] = CODE_XMR;
        this.codeSeparator = ' ';
        this.codePrefixed = true;
    }

    private XmrMonetaryFormat(char negativeSign, char positiveSign, char zeroDigit, char decimalMark, int minDecimals,
    ...
    /**
     * Parse a human readable coin value to a {@link org.bitcoinj.core.XmrCoin} instance.
     * 
     * @throws NumberFormatException
     *             if the string cannot be parsed for some reason
     */
    public XmrCoin parse(String str) throws NumberFormatException {
        return XmrCoin.valueOf(parseValue(str, XmrCoin.SMALLEST_UNIT_EXPONENT));
    }
