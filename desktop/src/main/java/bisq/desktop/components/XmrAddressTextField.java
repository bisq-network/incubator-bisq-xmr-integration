...
public class XmrAddressTextField extends AnchorPane {
    private static final Logger log = LoggerFactory.getLogger(XmrAddressTextField.class);

    private final StringProperty address = new SimpleStringProperty();
    private final StringProperty paymentLabel = new SimpleStringProperty();
    private final ObjectProperty<XmrCoin> amountAsCoin = new SimpleObjectProperty<>(XmrCoin.ZERO);
    ...
    public XmrAddressTextField(String label) {
        ...
        textField.setOnMouseReleased(event -> {
            if (wasPrimaryButtonDown)
                GUIUtil.showFeeInfoBeforeExecute(XmrAddressTextField.this::openWallet);
    ...
    public XmrCoin getAmountAsCoin() {
        return amountAsCoin.get();
    }

    public ObjectProperty<XmrCoin> amountAsCoinProperty() {
        return amountAsCoin;
    }

    public void setAmountAsCoin(XmrCoin amountAsCoin) {
    ...
    private String getMoneroURI() {
        if (amountAsCoin.get().isNegative()) {
            ...
            setAmountAsCoin(XmrCoin.ZERO);
        }
        //TODO(niyid) Return XMR URI
        return "";
    }
}
